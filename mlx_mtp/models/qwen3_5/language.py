...
import mlx.nn as nn
from mlx_mtp.models._activations import swiglu

from mlx_mtp.models.base import (
    LanguageModelOutput,
    create_attention_mask,
    scaled_dot_product_attention,
)
from mlx_mtp.models.cache import ArraysCache, KVCache
from mlx_mtp.models.rope import MRoPERotaryEmbedding
from mlx_mtp.models.rope import apply_multimodal_rotary_pos_emb as _apply_mrope
...
class LanguageModel(nn.Module):
    ...
    def make_mtp_cache(self):
        """Fresh stateless MTP KV cache for one D1 speculative round."""
        return self.mtp.make_cache() if hasattr(self, "mtp") else []

    def mtp_forward(self, hidden, token, mtp_cache):
        """Draft one token via the embedded MTP head (engine drafter call)."""
        return self.mtp.mtp_forward(hidden, token, mtp_cache)
